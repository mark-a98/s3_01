...
/****** Object:  StoredProcedure [dbo].[GetCarePlanProfileDetailsForCareplan_revamp]    Script Date: 2/19/2025 9:30:02 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[GetCarePlanProfileDetailsForCareplan_revamp]
	@episode_id BIGINT,
...
BEGIN
	...
	SELECT 1 AS isfromProfile, * FROM dbo.Func_GetAllCarePlanInterventionCommentByEpisodeId(@episode_id, -1, -1, -1,'', NULL)  ORDER BY COALESCE(visit_date, created_timestamp), CASE WHEN visit_date IS NOT NULL THEN created_timestamp END

	--CarePlan Care Goal Comment
	SELECT 1 AS isfromProfile, * FROM dbo.Func_GetAllCarePlanCareGoalCommentByEpisodeId(@episode_id, -1, -1, -1,'', NULL)  ORDER BY COALESCE(visit_date, created_timestamp), CASE WHEN visit_date IS NOT NULL THEN created_timestamp END
